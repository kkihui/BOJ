...
def vertical_1_judge(y,y1,y2,y3,y4):
    ...
    if L1_min - 1e-9 <= y and y <= L1_max + 1e-9 and L2_min - 1e-9 <= y and y <= L2_max + 1e-9:
        return 1
    else:
        return 0
...
def judge(x,y,x1,x2,x3,x4,y1,y2,y3,y4):
    ...
    if min(x1,x2)-1e-9 <= x and x <= max(x1,x2)+1e-9 and min(x3,x4)-1e-9 <= x and x <= max(x3,x4)+1e-9 and min(y1,y2)-1e-9 <= y and y <= max(y1,y2)+1e-9 and min(y3,y4)-1e-9 <= y and y <= max(y3,y4)+1e-9:
        return 1
    else:
        return 0 
...
def main():
    ...
    # L1만 세로선일 경우    
        ...
        # x1이 x3,x4 범위 안에 있고 x좌표에 해당하는 y값이 범위 안에 있을 때
        if min(x3,x4)<= x1 and x1 <= max(x3,x4) and vertical_1_judge(y,y1,y2,y3,y4):
            ans = 1
        else:
            ans = 0
    ...
    # L2만 세로선일 경우    
        ...
        # x3이 x1,x2 범위 안에 있고 x좌표에 해당하는 y값이 그 안에 있을 때
        if min(x1,x2)<= x3 and x3 <= max(x1,x2) and vertical_1_judge(y,y1,y2,y3,y4):
            ans = 1
        else:
            ans = 0
    ...
    # L1,L2 둘 다 대각선일 경우
        ...
        # 기울기가 일치한다면 (평행)
        if abs((y2-y1)/(x2-x1)-(y4-y3)/(x4-x3)) < 1e-9:
            L1_intercept = y2 - ((y2-y1)*x2/(x2-x1))
            L2_intercept = y4 - ((y4-y3)*x4/(x4-x3))
            # 절편 동일하고 범위도 일치할 때
            if abs(L1_intercept-L2_intercept) < 1e-9 and vertical_2_judge(y1,y2,y3,y4):
                ans = 1
            else:
                ans = 0 
