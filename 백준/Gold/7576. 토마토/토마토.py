...
class Graph:
    def __init__(self,row,col):
        self.tomato = ([[0]*col for _ in range(row)])
    ...
    def bfs(self): # 비재귀적으로 bfs 구현
        ...
        while not self.queue.empty(): # 큐가 빌 때까지 반복
            ...
            for k in range(4): # 4방향 탐색
                nr = now[0] + dr[k]
                nc = now[1] + dc[k]
                if 0<=nr<=self.row-1 and 0<=nc<=self.col-1 and self.tomato[nr][nc] == 0:
                    self.tomato[nr][nc] = 1
                    self.queue.put((nr,nc,now[2]+1))

        return now[2] # 마지막으로 처리한 길이가 최대임.
           
    def Wheter_all(self): # 토마토 다 익을 수 있는지 없는지 판단 (0인지 아닌지로)
        for i in range(self.row):
            for j in range(self.col):
                if self.tomato[i][j] == 0:
                    return False
        return True
