...
class maxheap:
    ...
    def insert(self,n): # 마지막에 삽입하고 부모와 비교하면서 교환
        ...
        if self.level > 1:
            while i > 1:
                if self.tree[i] > self.tree[i//2]: # 자식이 부모보다 크면 교환
                    temp = self.tree[i]
    ...
    def delmax(self): # 루트 노드 없애고 마지막 노드 넣고 다시 정렬
        if len(self.tree) == 1:
            print(0)
            return 0
            ...
            print(self.tree.popleft()) #max값 제거
        ...
        while 2*j <= self.level:
            if 2*j+1 <= self.level: # 자식이 둘 다 존재하는 경우
                if self.tree[2*j] >= self.tree[2*j+1]: #왼쪽 자식이 크거나 같을 때
                    if self.tree[j] < self.tree[2*j]: # 왼쪽 자식보다 작을 때 교환
                        temp = self.tree[j]
                    ...
                    if self.tree[j] < self.tree[2*j+1]: # 오른쪽 자식보다 작을 때 교환
                        temp = self.tree[j]
